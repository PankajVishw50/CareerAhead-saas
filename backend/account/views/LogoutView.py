    ...
    def post(self, request):
        ...
        if refresh_token:
            ...
            if not request.user.is_authenticated:
                return ErrorResponseTemplates.UNAUTHORIZED(
                    "Access token is required when refresh_token passed in cookie"
                )
        else:
        ...
        try:
            refreshtk = RefreshToken.objects.get(
                code=refresh_token, expiration_on__gt=now
            )
