...
def response_error(status_code=500, message="Something Went wrong", data={}):
    return Response(
        {
            "error": {
...
def _create_error_response(
    status_code: int = 500, message: str = "Something went wrong"
):
...
class ErrorResponseTemplates:
    ...
    UNAUTHORIZED = _create_error_response(
    ...
    METHOD_NOT_ALLOWED = _create_error_response(
    ...
    PAGINATION_NOT_FOUND = lambda max_page=None: _create_error_response(
        status.HTTP_404_NOT_FOUND,
        f"Invalid page." + f"Max page number is {max_page}" if max_page else "",
    )()
