    ...
    def validate(self, data):
        ...
        match data["account_type"]:
            ...
            case "vpa":
                if not data.get("vpa"):
                    errors["vpa"] = (
                        "You have to provide `vpa` payload when account_type is of `vpa`"
                    )

                if data.get("bank_account"):
                    errors["bank_account"] = (
                        "Invalid payload `bank_account` for account type `vpa`"
                    )

            case "bank_account":
                if not data.get("bank_account"):
                    errors["bank_account"] = (
                        "You have to provide `bank_account` payload when account_type is of `bank_account`"
                    )

                if data.get("vpa"):
                    errors["vpa"] = (
                        "Invalid payload `vpa` for account type `bank_account`"
                    )
            case _:
