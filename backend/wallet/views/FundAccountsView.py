...
class FundAccountsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            response = requests.get(
                ...
                headers={"Authorization": razorpay.auth_digest_header},
                params={
    ...
    def post(self, request):
        account = FundAccountSerializer(
            data={**request.data, "contact_id": request.user.wallet.contact_id}
        )

        if not account.is_valid():
            return ErrorResponseTemplates.BAD_REQUEST(
                message="Invalid Payload", data={"errors": account.errors}
            )
