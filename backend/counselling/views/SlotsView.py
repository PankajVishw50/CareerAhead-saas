...
class SlotsView(APIView, OrderingMixin):
    ...
    def get(self, request, counsellor_id):
        slots = (
            request.counsellor.slots.all_valids()
            .filter(timezone=request.counsellor.timezone)
            .order_by("from_time")
        )
    ...
    def post(self, request, counsellor_id):
        try:
            ...
            slots.sort(
                key=lambda x: datetime.datetime.strptime(
                    x["from_time"], settings.FORMAT_TIME
                )
            )
        except KeyError:
        ...
        if not slots_s.is_valid():
            return ErrorResponseTemplates.BAD_REQUEST(
                "Invalid payload", {"errors": slots_s.errors}
            )
        ...
        for i, slot in enumerate(slots_s.validated_data[1:]):
            last_end_time = (
                datetime.datetime.combine(
                    ...
                    slots_s.validated_data[i]["from_time"],
                )
                + slots_s.validated_data[i]["duration"]
            ).time()

            if last_end_time > slot["from_time"]:
                return ErrorResponseTemplates.CONFLICT(
                    "Slots are Conflicting." "Make sure no slots are overlapping"
                )

        # Check if slot count is not more than SLOT_MAX_COUNT
        if (
            total_slot_c := len(slots_s.validated_data)
            + request.counsellor.slots.count()
        ) > settings.SLOT_MAX_COUNT:
            return ErrorResponseTemplates.BAD_REQUEST(
    ...
    def delete(self, request, counsellor_id):
        ...
        if request.data.get("disabled"):
            ...
            if not isinstance(req_slots, list):
                return ErrorResponseTemplates.BAD_REQUEST(
                    "Invalid payload - `slots` must be array of slots id"
                )
