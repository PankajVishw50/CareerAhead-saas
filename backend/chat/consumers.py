...
def load_serialized_event(type=None):
    def decorator(func):
        def wrapper(self, event=None, text_data=None):
            ...
            event_data = event.get('text') if event and isinstance(event, dict) else text_data
...
class NotificationConsumer(WebsocketConsumer):
    ...
    def message_new(self, event):
        ...
        try:
            ...
            self.send(text_data=json.dumps(invalid_type_response("Chat does not exist")))
            return
        ...
        if not chat.user_owns_chat(self.user):
            self.send(text_data=json.dumps(invalid_type_response("You are not part of this chat")) )
            return 
        ...
        try:
            ...
            logger.exception(f"Failed to fetch chat by provided id with exception {e}", exc_info=e)
            self.send(text_data=json.dumps(invalid_type_response("Failed to send message")) )
            return

        # Send message to the chat users
        message_s = MessageSerializer(instance=message)
        response_data = serializer_uuid_dict(send_response("new_message", message_s.data))
    ...
    def message_seen(self, event):
        ...
        try:
            ...
            if not chat.user_owns_chat(self.user):
                return self.send(text_data=json.dumps(invalid_type_response("Forbidden")))

            # check if it's valid message
            message = chat.messages.get(id=self.event_data['payload']['id'])
            if message.sender == self.user: 
                return self.send(text_data=json.dumps(invalid_type_response("Cannot perform the action for specified id")))

            # Fetch all prior message to provided message which are not seen 
            messages = chat.messages.filter(created_at__lte=message.created_at, seen=False, sender=message.get_alternate_user(self.user))
        except Chat.DoesNotExist:
            return self.send(text_data=json.dumps(invalid_type_response("Chat not found")))
        except Message.DoesNotExist:
            return self.send(text_data=json.dumps(invalid_type_response("Message not found")))
        ...
        if message.sender.is_online:   
            async_to_sync(self.channel_layer.send)(
                message.sender.online_channel,
                {
                    "type": "send.client",
                    "text": json.dumps(send_response(
                        "messsage.seen",
                        {
